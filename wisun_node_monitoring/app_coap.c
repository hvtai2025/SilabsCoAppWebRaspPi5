...
// New: CoAP handler for /leds/control to turn LED ON/OFF
sl_wisun_coap_packet_t * coap_callback_leds_control(
      const sl_wisun_coap_packet_t *const req_packet) {
  char* payload_str = NULL;
  int led_action = -1; // 1 = ON, 0 = OFF
  if (req_packet->payload_len) {
    payload_str = sl_wisun_coap_get_payload_str(req_packet);
    if (payload_str != NULL) {
      if (sl_strcasecmp(payload_str, "LED ON") == 0) {
        led_action = 1;
      } else if (sl_strcasecmp(payload_str, "LED OFF") == 0) {
        led_action = 0;
      }
      sl_wisun_coap_free(payload_str);
    }
  ...
  if (led_action == 1) {
    ...
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "LED turned ON");
  } else if (led_action == 0) {
    sl_led_turn_off(&sl_led_led0);
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "LED turned OFF");
  } else {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "Invalid payload. Use 'LED ON' or 'LED OFF'.");
  }
  return app_coap_reply(coap_response, req_packet);
}
